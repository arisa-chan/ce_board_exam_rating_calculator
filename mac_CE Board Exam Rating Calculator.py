...
sg.theme("Material2")
sg.set_options(font=("Tahoma", 12))

layout_mst = [
    [sg.Text("MSTC", justification='l', font=("Tahoma", 12, "bold"))],
    [sg.Input(key="-SCORE_MSTC-", size=(2,2), justification='c', font=("Tahoma", 32)), sg.Text("/75")],
    [sg.Text("", key="-RATING_MSTC-", justification='c')]
]

layout_hge = [
    [sg.Text("HGE", justification='l', font=("Tahoma", 12, "bold"))],
    [sg.Input(key="-SCORE_HGE-", size=(2,2), justification='c', font=("Tahoma", 32)), sg.Text("/50")],
    [sg.Text("", key="-RATING_HGE-", justification='c')]
]

layout_psad = [
    [sg.Text("PSAD", justification='l', font=("Tahoma", 12, "bold"))],
    [sg.Input(key="-SCORE_PSAD-", size=(2,2), justification='c', font=("Tahoma", 32)), sg.Text("/75")],
...
while True :
    ...
    if event is None or event == "Exit" or event == sg.WIN_CLOSED :
        ...
        if rating_mstc < 50 or rating_hge < 50 or rating_psad < 50 or rating < 70 :
            window["-PASSED-"].update("FAILED", background_color="red", text_color="white", font=("Tahoma", 12,
                                                                                                  "bold"))
        else :
            window["-PASSED-"].update("PASSED", background_color="green", text_color="white", font=("Tahoma", 12,
                                                                                                    "bold"))
    elif event == "About" :
        sg.popup(
            "CE Board Exam Rating Calculator\n"
